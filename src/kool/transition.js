define(['jquery'], function ($) {
    ...
    function Transition() {
        ...
        function bindTransition($page, trans, isIn, callback) {
            ...
            $page.one(animationEndEvent, function (event) {
                ...
                if ($page.data('animate-num') < 0) {
                    alert('ERROR! animate num less than 0');
                }
                if ($page.data('animate-num') === 0) {
        ...
        self.transitBetween = function ($from, $to, transName, reverse, callbacks) {
            ...
            if ($from && $from.length > 0) bindTransition($from, trans, false, callbacks.inCallback);
            if ($to && $to.length > 0) bindTransition($to, trans, true, callbacks.outCallback);
        }
    }
});